...
@section('title', 'Balance Sheet Detail')

@section('content')
    <div class="mb-4">
        <h1 class="text-xl font-semibold">Neraca (Balance Sheet Detail)</h1>
        <p class="text-sm text-gray-500">
            Saldo akhir per bulan — Tahun {{ $year }} (kolom {{ $year - 1 }} = saldo akhir tahun sebelumnya)
    ...
    <div class="bg-white rounded border border-odoo-border shadow-sm mb-4 p-4">
        <form method="GET" action="{{ route('accounting.reports.balance-sheet-detail') }}" class="flex flex-wrap gap-3 items-end">
            <div>
            ...
            <div class="flex items-center gap-2 pb-2">
                ...
                <label for="hide_zero" class="text-sm text-gray-600">Sembunyikan akun tanpa saldo</label>
            </div>
        ...
        <table class="odoo-table w-full text-sm min-w-[1400px]">
            <thead>
                <tr>
                    ...
                    <th class="sticky left-[100px] bg-gray-50 z-10 min-w-[240px]">Uraian</th>
            ...
            <tbody>
                @foreach ($report['rows'] as $row)
                    @php
                        ...
                        $isTotal = in_array($row['type'], ['total', 'subtotal']);
                    ...
                    <tr class="{{ $rowClass }}">
                        <td class="sticky left-0 bg-inherit z-10 font-mono text-xs whitespace-nowrap"
                            style="padding-left: {{ ($row['indent'] ?? 0) * 12 + 12 }}px">
                        ...
                        <td class="sticky left-[100px] bg-inherit z-10"
                            style="padding-left: {{ ($row['indent'] ?? 0) * 8 }}px">
                            {{ $row['label'] }}
                        </td>
                        @foreach ($report['columns'] as $column)
                            @php
                            ...
                            @endphp
                            <td class="text-right font-mono whitespace-nowrap {{ $isSection ? '' : '' }}">
                                @if ($isSection)
                                    —
                                ...
                                @else
                                    @if (! empty($row['account_id']))
                                        <a href="{{ route('accounting.general-ledger.index', [
                                            'account_id' => $row['account_id'],
                                            'date_from' => $column['start_date'] ?? ($year - 1) . '-01-01',
    ...
    <div class="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div class="bg-white rounded border border-odoo-border p-3 text-sm">
            <div class="text-xs text-gray-500 uppercase">Total Kewajiban</div>
            <div class="font-mono font-semibold">
                {{ number_format($report['totals']['liabilities'][$lastCol['key']] ?? 0, 2, ',', '.') }}
            </div>
        </div>
        <div class="bg-white rounded border border-odoo-border p-3 text-sm">
